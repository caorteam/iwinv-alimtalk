...
링크 후 다음 명령으로 실행할 수 있습니다.

```sh
iwinv-alimtalk --help
```

링크하지 않고 실행하려면 다음 명령을 사용하세요.
...
소스는 `src/` 아래의 strict TypeScript이며 테스트는 `test/` 아래의 TypeScript입니다. 빌드 결과는 `dist/`에 생성되고 패키지 binary는 `dist/src/cli.js`를 가리킵니다.
...
`npm test`는 TypeScript 빌드를 먼저 실행한 뒤 `dist/test`의 컴파일된 테스트를 실행합니다. 이 패키지는 런타임 의존성이 없으며 Node 내장 `node:test`, `assert`, `fetch`, `Buffer` API를 사용합니다.
